...
namespace Flutter.Embedding
{
    ...
    public class EventChannel
    {
        ...
        public EventChannel(string name, IBinaryMessenger messenger) : this(name, StandardMethodCodec.Instance, messenger)
        {
        }
    ...
    class IncomingStreamRequestHandler
    {
        ...
        private Task<byte[]> OnListen(MethodCall call)
        {
            if (_activeSink != null)
            {
                ...
                {
                    Trace.TraceError($"{_channel.Name}: Failed to close existing event stream. {e.Message}");
                }
            }
            try
            {
            ...
            {
                _activeSink = null;
                Trace.TraceError($"{_channel.Name}: Failed to open event stream. {e.Message}");
        ...
        private Task<byte[]> OnCancel(MethodCall call)
        {
            if (_activeSink != null)
            {
                ...
                {
                    Trace.TraceError($"{_channel.Name}: Failed to close event stream. {e.Message}");
                    return Task.FromResult(_channel.Codec.EncodeErrorEnvelope("error", e.Message, null));
                }
            }
            else
